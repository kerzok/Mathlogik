...
public class Axioms {
    ...
    static boolean isAxiom(ArrayList<Expression> tokensList) {
        Expression expression = tokensList.get(tokensList.size() - 1);
        ...
        if (isModusPonens(expression, tokensList)) return true;
        return false;
    }
    ...
    private static boolean isModusPonens(Expression expression, ArrayList<Expression> tokensList) {
        for (int i = tokensList.size() - 1; i >= 0; i--) {
            if (tokensList.get(i) instanceof OperationImplication && ((OperationImplication) tokensList.get(i)).getRightToken().equals(expression)) {
                for (int j = tokensList.size() - 1; j >= 0; j--) {
                    if (((OperationImplication) tokensList.get(i)).getLeftToken().equals(tokensList.get(j)))
                        return true;
                }
            }
        }
